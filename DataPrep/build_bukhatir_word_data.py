#!/usr/bin/env python3
"""
build_bukhatir_word_data.py — Generate per-word timestamps for Bukhatir's recitation.

Algorithm (identical to build_noreen_word_data.py):
...
Input:
    DataPrep/bukhatir_audio/{001-114}.mp3
...
AUDIO_DIR = os.path.join(SCRIPT_DIR, "bukhatir_audio")
...
OUTPUT_PATH = os.path.join(PROJECT_ROOT, "Niya", "Resources", "Data", "word_data_bukhatir.json")
...
def main():
    if not os.path.isdir(AUDIO_DIR):
        ...
        print("Run fetch_bukhatir_audio.py first.")
        sys.exit(1)
    ...
    # Retry failed surahs with the larger "small" model
    if failed:
        print(f"\n{len(failed)} surahs failed with base model, retrying with small model...")
        small_model = whisper.load_model("small", device="cpu")
        still_failed = []
        for surah_id in failed:
            print(f"\n  Retrying surah {surah_id} with small model...")
            result = process_surah(small_model, surah_id, word_data, surahs)
            if result is not None:
                output[str(surah_id)] = result
            else:
                still_failed.append(surah_id)
        failed = still_failed
